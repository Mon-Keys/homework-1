...
* Карточка в ленте (расширенная)
  ...
  * ❌ При нажатии на кнопку "Пожаловаться" на пк отрисовывается меню с возможными жалобами только на половину экрана
    Баг: Отрисовка жалоб в ленте на пк
    <div style="width: 100%; display: flex;">
    <img src="assets/feed/3.png" width=400 style="margin: 0 auto;">
    </div>
  * ✅ Отображается сообщение об окончании карточек в ленте
...
* Поле "Поиск"
  * ✅ При вводе символа "N", в списке совпадений отображаются совпадения с пользователями, у которых имя начинается на "N" или на "n"
  * ✅ При вводе слова "Natasha", в списке совпадений отображаются совпадение с пользователем "Natasha"
  * ✅ При вводе слова "natasha", в списке совпадений отображаются совпадение с пользователем "Natasha"
  * ✅ При вводе слова "леонид", в списке совпадений отображаются совпадение с пользователем "Леонид"
  * ✅ При вводе имени пользователя, с которым нет совпадения (например "abracadabra"), в списке совпадений ничего не отображается
  * ✅ При вводе пробелов, поиск не осуществляется, список совпадений не изменяется
  * ✅ При вводе длинного слова, не соответствующего имени пользователя, в списке совпадений ничего не отображается
...
* Чат
  * ✅ При нажатии на кнопку "Назад" возвращаемся к списку чатов
  ...
  * Поле "Сообщение"
    ...
    * ✅ Отправка сообщения по кнопке "Отправить" и по клавише Enter
...
* Поле "Почта"
  ...
  * ✅ При неуспешном вводе подсказка под полем становится красной
...
* ### <a name="Login"></a> Логин
  ...
  * Кнопка "Войти"
    ...
    * ✅ При попытке входа незарегистрированного пользователя появляется ошибка красного цвета над кнопкой: "Неверная почта или пароль"
    * ✅ При попытке входа с неправильным паролем появляется ошибка красного цвета над кнопкой: "Неверная почта или пароль"
...
* Поле "Дата рождения"
    ...
    * ✅ При вводе в поле дня больше 31 снизу выводится сообщение об ошибке (выделено красным)
    * ✅ При вводе в поле месяца больше 12 снизу выводится сообщение об ошибке (выделено красным)
    * ✅ При вводе в поле дня больше текущего года снизу выводится сообщение об ошибке (выделено красным)
    * ✅ При вводе нулей в любое поле снизу выводится сообщение об ошибке (выделено красным)
    * ✅ При вводе даты на 100 лет меньшей текущей снизу выводится сообщение об ошибке (выделено красным)
    * ✅ После не валидного ввода при анфокусе цвет поля темнеет
    * ✅ При успешном вводе цвет поля должен вернуться в изначальный
    * ✅ При копировании в поле должна работать валидация
    * ❌ Валидируется поле день при вводе 31 при месяцах с 30 днями, а также 29 февраля
    БАГ: валидацию проходят даты вида: 31 04 2021, 29 02 2021
      <div style="width: 100%; display: flex;">
...
* Поле "Ваш возраст"
    * ✅ При нажатии на один из селектов другой тускнет
* Поле "Расскажите о себе"
    * ✅ При пустом вводе ошибка не выдается
    * ✅ При нажатии на кнопку "Сохранить" во вкладке профиль поле изменяется остальные остаются неизменными
* Поле "Расскажите о себе"
    * ✅ При нажатии на кнопку "Раскрыть интересы" раскрываются сохраненные интересы
    * ✅ При изменении интересов, свертывании и обратном развертывании изменения остаются
* Поле "Кого вы ищите"
    * ✅ При нажатии на один из селектов другой тускнеет
* Поле "Аватары"
    * ✅ Невозможно удалить изображение, если оно одно
    ...
    * ❌ Быстрое добавление нескольких фотографий при медленном интернете
    БАГ: Сетится плейсхолдер с анимацией загрузки вместо загружаемых фотографий
      <div style="width: 100%; display: flex;">
      <img src="assets/settings/1.png" width=200 style="margin: 0 auto;"> </div>
* Кнопка "Сохранить"
